...
- Order log (generic): `build_order_log(events, initial_capital)` turns a strategy's raw `OrderEvent`s into finalized `OrderRow`s (13 fixed columns — the raw trade plus derived value-after, running net deposits, P&L €/%, equity exposure, cash quota, period return — plus an `asset_values` dict (each asset's post-trade worth) and an `asset_prices` dict (each asset's exchange close on the trade day), which the UI renders as one value column **and** one price column per basket asset). The per-asset breakdown is produced by `_asset_values(holdings, prices)` (units × price per asset column, NaN-priced → 0, summing to `assets_after`) and `_asset_prices(prices)` (the bare close per asset, NaN → None) inside each plugin's events generator. This is the **only** order-log code here and is strategy-agnostic; each strategy emits its own `OrderEvent`s inside its plugin. `run_backtest()` now returns a 3-tuple `(series, metrics, order_log)`; its built-in fallback path (strategy=None, not used by the UI) runs Buy & Hold via `simulate_lumpsum()` and returns `order_log=None`.
