...
**UI building blocks**
- `layout.py`: top-level layout only, calls `_basket_ui()` from `components.py`; below the chart/metrics it stacks the two per-basket order tables (`bt-orders-a`/`bt-orders-b`) in **always-visible** divs under "Basket A"/"Basket B" headings — **not** a `dbc.Tabs` pane: a Bootstrap tab pane is `display:none` until activated and Safari won't repaint callback-injected content in the active pane until a tab switch, so a freshly-rendered table stayed blank until toggling B→A. Always-visible divs render first-paint like the chart/metrics.
