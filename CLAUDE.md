...
- `components.py`: `_basket_ui`, `_render_basket_list`, `_metrics_table`, and the order-table pair driven by the `_ORDER_COLUMNS` spec: `_order_rows(orders)` formats the log into JSON display rows (`{column label: text}`, stored in `bt-orders-store` and shared by the table and the CSV/Excel download), and `_order_table_component(rows)` renders them as a **native HTML `<table class="order-table">`** inside a single `dcc.Markdown(dangerously_allow_html=True)` (not thousands of `html.Tr`/`html.Td` — those cost *seconds* of dash-renderer per-component render; native HTML also needs no JS layout measurement, so it paints first-time inside the tabs UI), or the 'No orders.' placeholder.
